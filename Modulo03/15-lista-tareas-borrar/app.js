...
import { showMenu, confirmProcess, readInput, listChoices } from './Helpers/inquirer.js';
  ...
  try {
    ...
    while (opt !== 0 && continueProcess) 
    {
      ...
      switch(opt) {
        ...
        case 1:
          const taskDescription = await readInput('Descripción: ', 'Debe ingresar una descripción');
        ...
        case 6:
          if (arrTasks.length === 0) console.log('No hay ninguna tarea cargada para borrar.');
          else {
            const optDelete = await listChoices('Borrar', arrTasks);
            confirm = await confirmProcess('¿Desea borrarlo? ');
            if (confirm.ok && optDelete.value !== 0) tasks.deleteTask(optDelete.value);
          }
        break;


        default:
          console.log("Opcion 5 o 0");
          break;
      ...
      if (opt !== 0) {
        confirm = await confirmProcess();
        continueProcess = confirm.ok;
      }
    }
